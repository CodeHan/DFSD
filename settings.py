...
TURING_ARCHS = {'Tesla V100', '2080 Ti','Tesla P100'}
MODEL_CLASSES = {
...
def parse_args():
    ...
    parser.add_argument("--add_task_tokens", type=str2bool, default=False)
    ...
    parser.add_argument("--output_hidden_states", type=str2bool, default=False)
    ...
    parser.add_argument("--embedding_emd", default=False, type=str2bool) 
    parser.add_argument('--emd_temp',type=float,default=1.0)
    parser.add_argument('--restore',type=str2bool,default=False)
    parser.add_argument('--restore_task', type=str, default='which task is restored')
    parser.add_argument('--restore_epoch', type=str, default='the epoch of student model')
    parser.add_argument('--teacher_epoch', type=str, default='the epoch of the teacher model')
    parser.add_argument('--restore_inplace', type=str2bool, default=False,help='resore the task which has not been train n_train_epochs')
    parser.add_argument("--joint_step", type=int, default=4,help='calculate loss every joint_step and then update model ')
    parser.add_argument("--gama", type=float, default=0.5,help='the weight of hda loss')
    parser.add_argument("--alpha", type=float, default=0.8,help='the weight of bootstrap loss')
    parser.add_argument("--first", type=str2bool, default=True,help='whether change the task,default True')
    parser.add_argument("--max_len",type=int,default=0)
    parser.add_argument("--jsloss", type=str2bool, default=False,help='whether use JSLoss in EMD')
    ...
    args.model_dir_root = os.path.join(args.model_dir_root, args.model_name,
            args.seq_train_type, "{}_{}_dil_{}_emdtemp_{}{}{}{}{}{}{}{}{}".format("_".join(args.tasks),
                    args.gen_lm_sample_percentage,
                    args.beta,
                    args.emd_temp,                    
                    '_gama%s' % args.gama if args.gama > 0 else '',
                    '_alpha%s' % args.alpha if args.alpha != 0.8 else '',
                    '_js%s'%args.joint_step if args.joint_step>1 else '' ,                
                    '_lr%s'%args.learning_rate if args.learning_rate!=6.25e-5 else '',
                    '_sep' if args.seperate else '',
                    '_real' if args.real_sample else '',
                    '_jsloss' if args.jsloss else '',
                    '_gen' if not args.add_task_tokens else '') if "lll" in args.seq_train_type else "_".join(args.tasks))
    ...
    model_config = config_class.from_pretrained(args.pretrain_path,output_hidden_states=args.output_hidden_states,output_attentions=args.output_attentions)
