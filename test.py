...
def test_one_to_one(task_load, task_eval, model, score_dict):
    ...
    test_qadata = QADataset(task_load,TASK_DICT[task_eval]["test"] , "test", SPECIAL_TOKEN_IDS[task_load]).sort()
    ...
    ep = model.ep
    results_path = os.path.join(model_dir,"qa_{}_{}.csv".format(task_eval,ep))
    if not args.debug:
...
def test_one_to_many(task_load):
    score_dicts = []
    index=[str(i+1) for i in range(args.n_train_epochs[task_load])]
    index.append('finish')
    device=torch.device('cuda:%s'%args.device_ids[0])
    for ep in index:#range(args.n_train_epochs[task_load]):
        model_dir = get_model_dir([task_load])
        final_name='finish'
        model_path = os.path.join(model_dir, 'model-{}'.format(ep))
        ...
        model = MODEL_CLASS(model_config).to(device).eval()
        ...
        logger.info("task: {}, epoch: {}".format(task_load, ep))
...
if __name__ == '__main__':
    ...
    if args.seq_train_type == "multitask":
        ...
        if args.unbound:
            ...
            for task_load in args.tasks[index:]:
                test_one_to_many(task_load)
